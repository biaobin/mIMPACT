...
% of an element.
% between two : all the parameter of an elemetn or a line are found.
...
for i=1:length(SXSTRING)
    % lattice file has change line sysmbol, still the same element
    if SXSTRING{i}(end)=='&'
        SXSTRING{i}=SXSTRING{i}(1:end-1);  % delete the &
    end
...
while i<length(SXSTRING)-2
    ...
    if SXSTRING{i}(end)==':' % new element or line
        ...
        SXSTRING{i}(1:end-1)=upper(SXSTRING{i}(1:end-1));  %capital symbol 
...
%                 name = SXSTRING{i}(1:end-1);
%                 def = [ def, ...
%                         name, ' = struct("Name","', name, '","Length",',    
